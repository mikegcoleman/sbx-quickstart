...
The guide uses **DevBoard** — a full-stack Next.js + FastAPI issue tracker included in
the `devboard/` folder alongside this file. DevBoard has real-world complexity: a
REST API, a Postgres database, JWT auth, tests, and a handful of intentional bugs and
unfinished features that make ideal exercises for Claude.
...
Open a terminal, navigate to the `devboard/` directory that came with this guide, and
launch Claude:
...
Running `sbx` with no arguments opens a live terminal dashboard:
...
That's fine for solo work, but gets messy if you want to review changes before they
land, or if you want to run multiple agents at the same time.

**Branch mode** gives Claude its own Git worktree and branch, isolated from your main
working tree. You keep working normally; Claude works on its branch; you merge when
you're happy.
...
```bash
cd ~/path/to/devboard
sbx run claude --branch claude/fix-pagination
```

`sbx` creates a worktree under `.sbx/` in your repo root. Claude's changes go there,
not into your `main` working tree.

### Let sbx name the branch for you
...
# Diff against main
git diff main..claude/fix-pagination

# Push and open a PR
cd .sbx/<sandbox-name>-worktrees/claude/fix-pagination
git push -u origin claude/fix-pagination
gh pr create
```
...
```bash
sbx rm claude-devboard
# Removes the VM, worktree, and branch
```

> **Tip**: Use branch mode any time you want a safety net or a clean PR history.
> The exercises below work in either mode.
...
**Start a named sandbox in branch mode:**
...
Use pytest with verbose output: pytest tests/ -v
```
...
3. `pytest tests/ -v`
4. Report the results — including the failures planted in the codebase
...
With the failing tests identified, ask Claude to fix them.
...
While Claude is working, open the file in your editor on the **host**. You'll see
...
**Start (or reconnect to) your sandbox:**
...
# devboard-compose    claude  running  127.0.0.1:8080->8000, 127.0.0.1:3001->3000  /…/devboard
...
> Most dev servers default to `127.0.0.1`. When asking Claude to start a server,
> add: *"make sure it binds to 0.0.0.0 so it's reachable via port forwarding."*
...
Once implemented, the frontend search bar (in `src/app/projects/[id]/page.tsx`)
...
The `PROXY` column matters: `forward` means credentials were injected (only available
for AI API calls); `transparent` means policy was enforced but no credential injection.
...
### Try "Locked Down" mode for the next exercise
...
Then try running the sandbox. Claude won't be able to reach `api.anthropic.com` until
...
```bash
sbx run claude ~/devboard/backend ~/devboard/frontend:ro --name devboard-full
```

- `~/devboard/backend` — primary workspace (read/write); agent starts here
- `~/devboard/frontend:ro` — mounted read-only; agent can read but not write
...
Review the frontend search flow in ~/devboard/frontend/src and make sure the
...
`sbx exec` opens a shell (or runs a command) inside a running sandbox. Use it to:

- Inspect the environment Claude is working in
- Check Docker container status
- Run a quick test or command manually
- Install a tool that the agent doesn't know it needs
...
```bash
# Non-interactive (-d flag)
sbx exec -d devboard-compose bash -c "cd /path/to/devboard/backend && pytest tests/ -v --tb=short"
```
...
```bash
sbx create claude ~/devboard --name devboard-ci
...
```bash
sbx run claude ~/devboard --name agent-search --branch claude/search
sbx run claude ~/devboard --name agent-notifications --branch claude/notifications
...
sbx exec -it my-sb bash                 # interactive shell inside sandbox
sbx exec -d my-sb bash -c "pytest -v"  # run command non-interactively
...
# ── Dashboard ──────────────────────────────────────────────────────────────────
sbx                                     # open interactive TUI dashboard
...
The network policy is blocking the domain. Check what's being blocked:
